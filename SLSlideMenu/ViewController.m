...
- (void)slideMenu:(SLSlideMenu *)slideMenu prepareSubviewsForMenuView:(UIView *)menuView {
    ...
    if (slideMenu.direction == SLSlideMenuDirectionTop) {
        UILabel *lb = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, 100, 30)];
        lb.text = @"自定义控件1";
    ...
    if (slideMenu.direction == SLSlideMenuDirectionLeft) {
        if (slideMenu.direction == SLSlideMenuDirectionLeft) {
            _personView = [[BPPersonView alloc]initWithFrame:CGRectMake(0, 0, 200, self.view.frame.size.height)];
            ...
            perView.block = ^(NSInteger tag) {
                if(tag == 0){
                    ...
                    ViewController1 *exvc = [[ViewController1 alloc]init];
